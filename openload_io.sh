...
# Same prototype as translate_size, pretends KB/MB/GB are KiB/MiB/GiB
# translate_size multiplying the fractional part by 1000 rather than 1024
# does not give the most correct result as is, but the error is small.
translate_size_bin(){
    local REPLACED
    REPLACED=$(echo $1 | replace "KB" "KiB" | replace "MB" "MiB" | replace "GB" "GiB")
    translate_size "$REPLACED"
}
...
openload_io_probe() {
    ...
    if [[ $REQ_IN = *s* ]]; then
        FILE_SIZE=$(parse_tag 'class="count"' span <<< "$PAGE") && \
            translate_size_bin "$FILE_SIZE" && REQ_OUT="${REQ_OUT}s"
    fi
